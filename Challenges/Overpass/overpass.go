...
//Hàm ROT47 nhận vào 1 chuỗi và biến đổi nó 
func rot47(input string) string {
	var result []string
	for i := range input[:len(input)] {
		j := int(input[i])
		if (j >= 33) && (j <= 126) { //Kiểm tra kí tự có trong khoảng 33 ->126(đảm bảo có thể hiển thị)
...
//thiết lập password cho service (nếu tồn tại đổi mk)
...
func main() {
	//Lấy đường dẫn đến file .overpass 
	credsPath, err := homedir.Expand("~/Shared/Overpass/.overpass")
	if err != nil {
	...
	case 1:
		...
		getPwdForService(passlist, service) //Lấy password nhưng ko in ra 
	case 2:
		service := input("Enter Service Name:\t")
